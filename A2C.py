...
class Critic():
    def __init__(self, sess, state_dim):
        ...
        # produced by the neural network as well
        ...
        # calculate self.v reward-to-go
        self.v = self.nn(self.state_input_ph)

        # calculate self.loss
        y_labels = self.reward_ph + GAMMA * self.v_next_ph
        self.loss = tf.keras.losses.MSE(self.v, y_labels)

        # calculate self.td_error
        self.td_error = y_labels - self.v
        ######### Your code ends here #########
