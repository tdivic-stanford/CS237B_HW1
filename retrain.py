...
def get_bottleneck_dataset(model, img_dir, img_size):
    ...
    for i in range(train_img_gen.samples):
        ...
        # get the prediction for the image from the bottleneck model
        prediction = model(x_i)

        # append to the proper arrays
        bottelneck_x_l.append(prediction)
...
def retrain(image_dir):
    ...
    # get the size of the bottleneck tensor
    input_size = bottleneck_train_ds.element_spec[0].shape[0]

    # create a new keras input
    retrain_input = tf.keras.layers.Input(shape=(input_size,))

    # define a trainable layer
    retrain_layer = tf.keras.layers.Dense(3, name="classifier", activation='softmax')
