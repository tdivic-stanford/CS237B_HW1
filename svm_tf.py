...
def circle_phi(x):
    ...
    # add a column that is sign(x1*x2)
    new_col = np.transpose(np.array(np.sign([x[:,0] * x[:,1]])))
...
def inner_circle_phi(x):
    ######### Your code starts here #########
    # add a column that is x1^2 + x2^2
    new_col = np.transpose(np.array([np.square(x[:,0]) + np.square(x[:,1])]))
    x = np.append(x, new_col, axis=1)
    ######### Your code ends here #########
    return x
...
class SVM(tf.keras.Model):
    ...
    def call(self, x, is_prediction=True):
        ...
        ######### Your code starts here #########
        if is_prediction:
            # if a prediction, calculate the output label as y_est = sgn(xW-b)
            y_est = tf.sign(tf.subtract(tf.matmul(x, self.W), self.b))
        else:
            # if not a prediction, compute the actual value of y_est
            y_est = tf.subtract(tf.matmul(x, self.W), self.b)
...
def loss(y_est, y, W, lam):
    ...
    ######### Your code starts here #########
    # calculate the hinge loss first
    hinge_loss = tf.keras.losses.hinge(y, y_est)

    # add the margin minimization term
    loss = hinge_loss + lam * tf.square(tf.norm(W))
    ######### Your code ends here #########
    return loss
...
def svm(data, basis_function=identity_phi, epochs=10, ret_dec_p=False):
    ...
    def train_step(x, y):
        ...
        with tf.GradientTape() as tape:
            # make a forward pass
            y_est = svm_model.call(x, is_prediction=False)
        ...
        grads = tape.gradient(current_loss, svm_model.trainable_variables)

        # run an optimization step on the weights
        optimizer.apply_gradients(zip(grads, svm_model.trainable_variables))
    ...
    def eval_step(x, y):
        ...
        y_preds = svm_model.call(x, is_prediction=True)

        # add values to respective metrics
        eval_loss(current_loss)
        eval_accuracy(y, y_preds)
        ######### Your code ends here #########
...
def get_hog_data():
    ...
    ######### Your code starts here #########
    # create quick helper function that extracts desired data and corresponding labels
    def extract_data_and_labels(key, y_label):
        x_data = hog_descriptor(pedestrian_data[key]).numpy()
        y_data = y_label * np.ones((x_data.shape[0], 1))
        return x_data, y_data

    # get the training data
    x_train_pos, y_train_pos = extract_data_and_labels("train_pos", 1)
    x_train_neg, y_train_neg = extract_data_and_labels("train_neg", -1)
    x_train = np.vstack((x_train_pos, x_train_neg))
    y_train = np.vstack((y_train_pos, y_train_neg))

    # get the eval data
    x_eval_pos, y_eval_pos = extract_data_and_labels("eval_pos", 1)
    x_eval_neg, y_eval_neg = extract_data_and_labels("eval_neg", -1)
    x_eval = np.vstack((x_eval_pos, x_eval_neg))
    y_eval = np.vstack((y_eval_pos, y_eval_neg))

    # get the true data
    x_test_pos, y_test_pos = extract_data_and_labels("test_pos", 1)
    x_test_neg, y_test_neg = extract_data_and_labels("test_neg", -1)
    x_pred = np.vstack((x_test_pos, x_test_neg))
    y_true = np.vstack((y_test_pos, y_test_neg))
